...
from game.models import QuestionOfficeGame, AnswerOfficeGame, QuestionTraining, AnswerTraining, KmbQuestionOffice, KmbAnswerOffice, UserResultsAccess, UserResultsKmbAccess
...
def game(request):
    question = QuestionOfficeGame.objects.filter(
        id__gt=request.user.profile.last_question_id).first()

    if question is None:
        answers = AnswerOfficeGame.objects.filter(
    ...
    if request.method == "POST":
        ...
        question = get_object_or_404(QuestionOfficeGame, pk=question_id)

        existing_answer = AnswerOfficeGame.objects.filter(
        ...
        if existing_answer:
            ...
            AnswerOfficeGame.objects.create(
                user=request.user,
                question=question,
...
def user_results(request):
    ...
    user_answers = AnswerOfficeGame.objects.filter(user=request.user)
...
def resultinfo(request):
    ...
    correct_answers = AnswerOfficeGame.objects.filter(
        user=user, right_answer=F('answer')).count()

    total_questions = QuestionOfficeGame.objects.count()
...
def game_kmb(request):
    ...
    if request.method == "POST":
        selected_answer = request.POST.get('answer')
        question_id = request.POST.get('question_id')
        question = get_object_or_404(KmbQuestionOffice, pk=question_id)

        existing_answer = KmbAnswerOffice.objects.filter(
            user=request.user, question=question).first()
        ...
        if existing_answer:
            ...
            KmbAnswerOffice.objects.create(
                user=request.user,
                question=question,
                answer=selected_answer,
...
def resultinfo_kmb(request):
    correct_answers = KmbAnswerOffice.objects.filter(
        user=request.user, right_answer=F('answer')).count()

    total_questions = KmbQuestionOffice.objects.count()
...
def user_results_kmb(request):
    ...
    user_answers = KmbAnswerOffice.objects.filter(user=request.user)
